...
namespace CR_TRIGGERS {
	...
	TRIGGER_PARAM_DEF ALL_TRIGGER_DEF::all_params[] {
		...
		TRIGGER_PARAM_DEF(KW_USE_INITIAL_BUILDING_STATUS, TPT_INTEGER, "use_initial_building_status", "Set to 1 to enable (use the \"non-constructed\" status for building unit), 0 to disable (fully-build unit)")
	};
	...
	TRIGGER_ACTION_DEF ALL_TRIGGER_DEF::actions[] = {
		...
		TRIGGER_ACTION_DEF(TYPE_MODIFY_UNIT, "modify_unit", std::vector<TRIGGER_KWID_PARAMS>({ KW_ACTION_UNIT_ID, KW_ADD_HP, KW_SET_HP, KW_ADD_RESOURCE, KW_SET_RESOURCE, KW_UNIT_GRAPHICS_AGE, KW_UNIT_ORIENTATION_INDEX, KW_POS_X, KW_POS_Y, KW_OWNER_PLAYER_ID }),
			...
			TRIGGER_ACTION_DEF(TYPE_ADD_UNIT_INSTANCE, "spawn_unit", std::vector<TRIGGER_KWID_PARAMS>({ KW_ACTION_PLAYER_ID, KW_ACTION_UNIT_DEF_ID, KW_UNIT_NAME, KW_POS_X, KW_POS_Y, KW_USE_INITIAL_BUILDING_STATUS }), "Spawns a unit, given a unit definition ID (see AGE3), for a given player, at provided location. \nFor unit \"definitions\" that are added by another trigger (see 'add_unit_def'), please provide unit_name instead of ID. \nWarning: the \"unit name\" param is only used to find unit when it has been created by another trigger.\n\"use_initial_building_status\" can only be used to create buildings (type80)."),
	...
	void ALL_TRIGGER_DEF::static_check() {
		// Here are 3 static checks to make sure event/action/param lists are consistent between enumeration and the setup above.
