...
void WxInstallCustomROR::ConstructorInit(std::wstring EXEFileName) {
	...
	this->MainArea->Add(new wxTextCtrl(this, wxID_ANY, _T("This screen allows you to install customROR (requires ROR 1.0a).\n")
		...
		_T("When you're ready, click on OK and CustomizeAOE will install customROR mod automatically.\n\n")
		_T("Tip: after installation, open customROR.xml and customROR_civs.xml in a text editor (as admin if necessary) to configure customROR and additional civilzations. These files will be located in your game installation directory.\n"),
...
{
	if (this->gameFileName.empty()) {
		wxMessageBox(_T("Please first select game EXE file to patch."), _T("Install customROR"), wxICON_WARNING | wxOK, this);
		return;
	}
	...
	if (this->hasMissingFile) {
		wxMessageBox(_T("Some installation files are missing. Make sure you selected the correct file and that you extracted all files from the installation archive."), _T("Install customROR"), wxICON_WARNING | wxOK, this);
		return;
	}
