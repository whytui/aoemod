...
void WxEditFile::CreateDetailedNodeContent(BINSEQ_CATEGORIES category) {
	...
	while (seqdefIndex > -1) {
		...
		try {
			...
			wxMessageBox(message, "Customize AOE", wxOK | wxICON_ERROR, this);
			return;
		}
...
void WxEditFile::ResetNodeToDefault() {
	...
	if (data->category == BC_NONE) {
		wxMessageBox("Cannot reset to default in this screen", "Customize AOE", wxOK | wxICON_INFORMATION);
		return;
	}
...
void WxEditFile::ShowNodeContent(wxControl *focusedControl) {
	...
	if ((data->category == BC_TECH_FIXES) && !data->isAdvanced) {
		try {
			...
			wxMessageBox("An error occurred. Maybe this file version is not fully supported.", "Customize AOE", wxOK | wxICON_ERROR, this);
			newCbxStatus = wxCheckBoxState::wxCHK_UNCHECKED;
		}
	}
...
void WxEditFile::DoROR_API_checks() {
	...
	if (error) {
		...
		wxMessageBox(message, "Customize AOE", wxOK | wxICON_WARNING, this);
	}
}
...
void WxEditFile::OnEnableFeatureClick(wxCommandEvent& event) {
	...
	case BC_RESOLUTION:
		...
		if (enable) {
			x = (unsigned short int) wxGetNumberFromUser("Please provide the new Horizontal resolution value", "Horizontal resolution", "Customize AOE", 1024, 0, 50000, this);
			if (x <= 0) {
				cancelled = true; 
				break;
			}
			y = (unsigned short int) wxGetNumberFromUser("Please provide the new Vertical resolution value", "Vertical resolution", "Customize AOE", 768, 0, 50000, this);
			if (y <= 0) {
				cancelled = true; 
				break;
			}
	...
	case BC_SELECTED_UNITS:
		...
		if (enable) {
			nb = (unsigned short int) wxGetNumberFromUser("Please provide a new value", "Maximum number of units that can be selected at once", "Customize AOE", 25, 0, 255, this);
			if ((nb <= 0) || (nb > 255)) {
				cancelled = true;
				break;
			}
...
void WxEditFile::OnValueLinkClick(wxCommandEvent& event) {
	try {
		...
		std::wstring strResult = wxGetTextFromUser("Please type variable value", "Customize AOE", sTmp, this, -1, -1, true);
		...
		wxMessageBox("The value is incorrect", "Customize AOE", wxOK | wxICON_ERROR, this);
	}
	catch (std::out_of_range) {
		wxMessageBox("The value is out of range", "Customize AOE", wxOK | wxICON_ERROR, this);
	}
	catch (std::exception &e) {
		wxMessageBox(e.what(), "Customize AOE", wxOK |wxICON_ERROR, this);
	}
	catch (...) {
		wxMessageBox("Unknown exception", "Customize AOE", wxOK | wxICON_ERROR, this);
	}
}
