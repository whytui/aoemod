...
bool CustomizeAOE_base::CheckAndRunAutoFixes(bool noPromptBeforeFix) {
	...
	if (techFixesStatus < 0) {
		...
		// This can happen when user is migrating from an old version of customROR/CustomizeAOE.
		this->DisplayMessage(_T("Some technical fixes have an invalid status...\n"));
		if (hasObsolete) {
			if (this->AskForConfirmation(_T("\nCustomizeAOE can try to fix incompatibility by removing obsolete changes. Let customizeAOE fix obsolete changes (Y/N) ? "))) {
				...
				this->DisplayMessage(_T("Aborting due to inconsistent status. Try again and answer yes to let CustomizeAOE try to fix EXE file...\n"));
				return false;
			}
		} else {
