...
void AOE_binData::InitTechFixes_10c() {
	...
	NEXT_INITSEQ_2_NOVAR(this->techFixes_10c.GetBinSeqDefinition(i),
		FixDiamMapInitXYInversion,
		"Fix a bug that leads to bad minimap display and random crash when creating non-square maps.",
		0x12122C,
		(0x8B, 0x68, 0x08, 0x8B, 0x50, 0x0C, 0x83, 0xC8, 0xFF), // default=no
		(0x8B, 0x68, 0x0C, 0x8B, 0x50, 0x08, 0x83, 0xC8, 0xFF),
		FM_OFF,
		FM_ON
		);
...
void AOE_binData::InitROR_API_10c() {
	...
	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		...
		0x0060512,
		(0x74, 0x22, 0x0F, 0xBF, 0x86, 0xF6, 0x01, 0x00, 0x00, 0x8B, 0x11), // default=no
		(0x74, 0x22, 0xE8, 0x5B, 0x90, 0xFB, 0xFF, 0x90, 0x90, 0x8B, 0x11),
		FM_OFF,
		FM_ON
		);
	...
	*/
	...
	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		UnitUpdateVisibilityToOtherPlayers,
		"Entry point on unit 50+ visibility update. Warning: if not used correctly, this may cause performance issues",
		0x26C86,
		(0x0F, 0x84, 0x9D, 0x00, 0x00, 0x00, 0x8D, 0x96, 0x8C, 0x01, 0x00, 0x00, 0x52),
		(0x0F, 0x84, 0x9D, 0x00, 0x00, 0x00, 0xE8, 0xE3, 0x28, 0xFF, 0xFF, 0x90, 0x52),
		FM_OFF,
		FM_ON
		);
	...
	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		...
		0xD1BB1,
		(0x89, 0x44, 0x24, 0x14, 0x8B, 0x86, 0x1C, 0x0E, 0x00, 0x00, 0x83, 0xF8, 0x15),
		(0x89, 0x44, 0x24, 0x14, 0xE8, 0xBA, 0x79, 0xF4, 0xFF, 0x90, 0x83, 0xF8, 0x15),
		FM_OFF,
		FM_ON
		);
	...
	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		ActionBuildFarmHack,
		"Overrides the farm hack in actionBuild update",
		0xB1AF0,
		(0x85, 0xC0, 0x0F, 0x84, 0x05, 0xFF, 0xFF, 0xFF, 0x8B, 0x40, 0x08),
		(0x85, 0xC0, 0xE8, 0x7D, 0x7A, 0xF6, 0xFF, 0x90, 0x8B, 0x40, 0x08),
		FM_OFF,
		FM_ON
		);

	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		OnAddInUnitMemoryList,
		"Update internal cache when a unit is added in unit memory list",
		0xBD7CF,
		(0x8B, 0xCB, 0xE8, 0x0A, 0x00, 0x00, 0x00, 0x5F, 0x5E, 0x5B, 0xC2, 0x04, 0x00, 0x90, 0x90, 0x90, 0x90),
		(0x8B, 0xCB, 0xE8, 0x9E, 0xBD, 0xF5, 0xFF, 0x83, 0xC4, 0x38, 0x5F, 0x5E, 0x5B, 0xC2, 0x04, 0x00, 0x90),
		FM_OFF,
		FM_ON
		);

	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		OverloadIsGamePaused,
		"Entry point to allow some actions when game is paused",
		0x199BF,
		(0x90, 0x83, 0xB9, 0xB0, 0x01, 0x00, 0x00, 0x07, 0x75, 0x03),
		(0x90, 0xE8, 0xAF, 0xFB, 0xFF, 0xFF, 0x85, 0xC0, 0x75, 0x03),
		FM_OFF,
		FM_ON
		);

	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		ExecMoveCmdUsingFormation,
		"Overloads the choice to use (or not use) formation movement for multiple selected units move",
		0x2A959,
		(0xF6, 0xC4, 0x41, 0x0F, 0x84, 0x35, 0x02, 0x00, 0x00, 0xD9, 0x44, 0x24, 0x28),
		(0xF6, 0xC4, 0x41, 0xE8, 0x13, 0xEC, 0xFE, 0xFF, 0x90, 0xD9, 0x44, 0x24, 0x28),
		FM_OFF,
		FM_ON
		);

	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		unitGroupTask2_forceOnConvert,
		"Fix the 'force' flag handling for other tasks than 'attack' in unitGroupTask(case 2)",
		0xCDE95,
		(0x8B, 0xCB, 0xE8, 0xC4, 0x9B, 0xF3, 0xFF, 0x8B, 0xC8),
		(0x8B, 0xCB, 0xE8, 0xD8, 0xB6, 0xF4, 0xFF, 0x8B, 0xC8),
		FM_OFF,
		FM_ON
		);

	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		playerAddOneUnitToSelection,
		"Entry point when adding a unit to player selection",
		0x5DC25,
		(0x0F, 0xBF, 0xCA, 0x8B, 0xB4, 0x88, 0x60, 0x08, 0x00, 0x00, 0x85, 0xF6),
		(0x0F, 0xBF, 0xCA, 0xE8, 0x47, 0xB9, 0xFB, 0xFF, 0x90, 0x90, 0x85, 0xF6),
		FM_OFF,
		FM_ON
		);
	...
	NEXT_INITSEQ_2_NOVAR(this->ROR_API_10c.GetBinSeqDefinition(i),
		deleteFile,
		"Entry point when deleting a file",
		0x126258,
...
void AOE_binData::InitObsoletes() {
	...
	NEXT_INITSEQ_2_NOVAR(this->obsoletes_10c.GetBinSeqDefinition(i),
		OBSOLETE_ManageAllOnButtonClickEvents,
		"Old version for buttonclick events, caused issues with checkboxes and radiobuttons.",
		0x00604C3,
	...
	NEXT_INITSEQ_2_NOVAR(this->obsoletes_10c.GetBinSeqDefinition(i),
		OBSOLETE_ManageOnDialogUserEvent,
