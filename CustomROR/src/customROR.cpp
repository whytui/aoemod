...
void CustomRORInstance::ReadTextFromChat(REG_BACKUP *REG_values) {
	...
	if (strcmp(txt, "seed") == 0) {
		this->crCommand.PrintMapSeed();
	}
	if ((strcmp(txt, "t") == 0) || (strcmp(txt, "time") == 0)) {
		this->crCommand.PrintDateTime();
	}
	if ((strcmp(txt, "ai") == 0) || (strcmp(txt, "AI") == 0)) {
		crCommand.RestoreAllAIFlags();
		CallWriteText("AI flags have been recomputed");
	}
	if ((strcmp(txt, "allai") == 0) || (strcmp(txt, "allAI") == 0)) {
		crCommand.SetAllAIFlags();
		CallWriteText("All players are now computer-managed.");
	}
	if (strcmp(txt, "dump") == 0) {
		this->crCommand.DumpDebugInfoToFile();
	}
	...
	if (strcmp(txt, "timer stats") == 0) {
		this->crCommand.DisplayTimerStats();
	}

#ifdef _DEBUG
	// p0 to p8: quick player change + set AI flags automatically
	if ((*txt == 'p') && (*(txt + 1) >= '0') && (*(txt + 1) < '9')) {
		int playerId = *(txt + 1) - '0';
		this->crCommand.ChangeControlledPlayer(playerId, true);
	}
#endif
	...
	if (strcmp(txt, "v") == 0) {
		...
		//this->crCommand.CheckAIWhenEnablingAIControl(1);
		/*ROR_STRUCTURES_10C::STRUCT_AI *ai = GetPlayerStruct(1)->GetAIStruct();
		if (ai) {
			ai->structTradeAI.hasDiplomacyInitBeenDone = 1;
		}*/
...
void CustomRORInstance::PlayerCreateUnit_manageStatus(REG_BACKUP *REG_values) {
	...
	char desiredStatus = *((char*)REG_values->ESP_val + 0x24); // get arg5 (byte)
	...
	if (settings->currentUIStatus == AOE_CONST_INTERNAL::GAME_SETTINGS_UI_STATUS::GSUS_IN_EDITOR) {
		...
		if ((desiredStatus > 2) && (unit->ptrStructDefUnit->unitType == AOE_CONST_FUNC::GLOBAL_UNIT_TYPES::GUT_BUILDING)) {
			float f = 10000;
			_asm {
				PUSH f
				MOV ECX, unit
