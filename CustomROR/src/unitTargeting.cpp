...
STRUCT_INF_AI_UNIT_LIST_ELEM *TestFindGroupMainTarget(STRUCT_INF_AI *infAI, long int targetPlayerId,
	...
	if (targetInfo->targetSearchInProgress) {
		return ContinueFindGroupMainTargetInProgress(infAI, targetPlayerId, unitGroup, targetInfo, baseTimeGetTimeValue);
	}
	...
	if (enemyWonderInfo) {
		...
		int index = (enemyWonderInfo - infAI->unitElemList);
		assert(index >= 0);
		...
		unitGroup->targetPlayerId = enemyWonderInfo->playerId;
		unitGroup->terrainZoneId = leaderTerrainZoneId;
		targetInfo->targetUnitId = enemyWonderInfo->unitId;
		targetInfo->targetEvaluation = 1000; // TODO ?
		targetInfo->targetInfAIUnitElemListIndex = index;
		targetInfo->targetSearchInProgress = 0;
		enemyWonderInfo->attackAttempts++;
	...
	if (hasStandardVictoryCondition && (targetHasAllRelics || targetHasAllRuins)) {
		// TODO: Disable other target if far from artefacts
		// TODO: find a target (visible) near artefacts...
		// TODO: attack towers/military units that protect them
	}
	
	// What do we do with current target, if any ?
	//GetInfAIElemForCurrentTargetIfStillEligible(infAI, targetPlayerId, unitGroup, targetInfo, baseTimeGetTimeValue);
