...
void MapCopyPopup::_AddPopupContent() {
	...
	this->AddLabel(popup, &unused, localizationHandler.GetTranslation(-1, "Copy/Paste map zones"), 100, currentVPos, 300, 20, AOE_FONT_BIG_LABEL);
	currentVPos += 35;
	// Copy pos
	this->AddLabel(popup, &unused, localizationHandler.GetTranslation(-1, "Copy from (x/y of bottom-left corner)"), 10, currentVPos, 250, 20, AOE_FONT_STANDARD_TEXT);
	...
	currentVPos += 30;
	this->AddLabel(popup, &unused, localizationHandler.GetTranslation(-1, "Copy to (x/y of top-right corner)"), 10, currentVPos, 250, 20, AOE_FONT_STANDARD_TEXT);
	...
	currentVPos += 30;
	// Paste pos
	this->AddLabel(popup, &unused, localizationHandler.GetTranslation(-1, "Paste to position (x/y of bottom-left corner)"), 10, currentVPos, 250, 20, AOE_FONT_STANDARD_TEXT);
	...
	currentVPos += 30;
	this->AddLabel(popup, &unused, localizationHandler.GetTranslation(-1, "Copy units"), 10, currentVPos, 250, 20, AOE_FONT_STANDARD_TEXT);
	...
	currentVPos += 60;
	this->AddButton(popup, &this->btnCopyAllMap, "Copy all map", 30, currentVPos, btnSize, 30, 0);
	this->AddButton(popup, &this->btnCopyZone, "Copy zone", 30 + btnSize + 10, currentVPos, btnSize, 30, 0);
	this->AddButton(popup, &this->btnPaste, "Paste", 30 + btnSize * 2 + 20, currentVPos, btnSize, 30, 0);
...
std::string MapCopyPopup::GetBufferSizeText() {
	std::string bufferInfoText = "Current buffer: size=";
	bufferInfoText += std::to_string(MAP::mapCopierInstance.GetCopiedSizeX());
	bufferInfoText += ", ";
	bufferInfoText += std::to_string(MAP::mapCopierInstance.GetCopiedSizeX());
	bufferInfoText += ". Contains units: ";
	bufferInfoText += MAP::mapCopierInstance.BufferContainsUnits() ? "yes" : "no";
...
bool MapCopyPopup::OnButtonClick(ROR_STRUCTURES_10C::STRUCT_UI_BUTTON *sender) {
	...
	if (sender == this->btnCopyZone) {
		...
		if (this->edtResultMessage->pTypedText != NULL) {
			AOE_SetEditText(this->edtResultMessage, result ? "success" : MAP::mapCopierInstance.GetLastError().c_str());
		}
