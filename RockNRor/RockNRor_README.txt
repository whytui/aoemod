...
- Run RockNRorAdmin and click Menu>File>Install RockNRor.
- Select C:\temp\RockNRor\ROR_API.dll and C:\your_aoe_install_dir\EmpiresX.exe (the game file you want to patch).
...
* Add up to 255 civilizations (total) + configure AI, names... (see RockNRor_civs.xml)
...
- Automatically assign shortcut numbers to spawned units (to configure in RockNRor.xml file).
...
- Show "remaining" available technologies (from you civ's tech tree) from RockNRor options UI (tech tree button).
...
- Use RockNRor.xml to configure (for RM/DM) initial resource values, initial resource bonus for AI
- Use RockNRor.xml to force certain PER numbers in DM and/or RM. Example: set SNAllowDiplomacyChangeOnAllyAttack to 0 to avoid unfortunate diplomacy changes.
- Enable/Disable (RockNRor.xml) neutral initial diplomacy stance: this prevents computer players to become all allied against human player.
...
- Disable dock in maps where AI does NOT build docks, for fair SP games. You can turn this ON/OFF in RockNRor.xml file.
...
- All units are visible in editor (can be turned off in RockNRor configuration file)
...
Other commands (in RockNRor options menu)
...
- Game may crash if RockNRor.xml content is invalid. Try to restart using provided RockNRor.xml example.
- RockNRor produces a log file named "RockNRor.log" in Age of Empires installation directory.
...
- Make sure the "ROR_API.conf" file contains the lines 'wndmode.dll' (if you use windowed mode) and 'RockNRor.dll' (without the quotes and without extra characters on the same line)
- Never declare more civilizations in RockNRor_civs.xml than there really exist in empires.dat. ROR_API will force civilization to Egyptian if you select a civilization that is not found in empires.dat.
- If you install RockNRor on a UPatch HD1.1 installation, you may need to empty the file gamex/up.ini to have windowed mode work correctly. Default file values from UPatch installation seem to be incorrect.
- If you install RockNRor on a directory game that already had RockNRor, config files will not be overwritten.
	To get latest features, you should really get RockNRor.xml content from the provided example (in ZIP archive file). Same for RockNRor_civs.xml.
