	...
	if ((notifyEvent->eventId == GAME_EVENT_TYPE::EVENT_ACTION_COMPLETED) || (notifyEvent->eventId == GAME_EVENT_TYPE::EVENT_ACTION_COMPLETED_SEARCH)) {
		...
		//if (activity->currentTaskId == ACTIVITY_TASK_ID::CST_ATI_TASK_BUILD) {
			...
			if (isHuman && ROCKNROR::crInfo.configInfo.autoGatherAfterBuildDeposit) {
				...
				bool wasBuildingTC = buildingValid && (buildingUnit->unitDefinition->DAT_ID1 == CST_UNITID_FORUM);
				if (villager->resourceValue == 0) {
					if (wasBuildingStoragePit || wasBuildingGranary || wasBuildingTC) {
					...
					if (wasBuildingTC ||
						(wasBuildingGranary && (villager->resourceTypeId == CST_RES_ORDER_BERRY_STORAGE)) ||
						(wasBuildingStoragePit && (
